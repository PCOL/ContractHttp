namespace ContractHttp.Reflection
{
    ...
    public static class ReflectionExtensions
    {
        ...
        {
            return type.GetMethod(name, genericArgumentCount, BindingFlags.Public | BindingFlags.Instance, argumentTypes);
        }
        ...
        /// <param name="argumentTypes">An array of argument types.</param>
        /// <returns>A <see cref="MethodInfo"/> if the method is found; otherwise false.</returns>
        public static MethodInfo GetMethod(this Type type, string name, int genericArgumentCount, BindingFlags bindingFlags, params Type[] argumentTypes)
        {
            IEnumerable<MethodInfo> methodInfos = type.GetMethods(bindingFlags).Where(m => m.Name == name);
                ...
                foreach (var a in AssemblyCache.GetAssemblies())
                {
                    if (a.IsDynamic == false)
                    {
                        methodInfos = a.GetTypes()
                            ...
                            .SelectMany((t) => t.GetMethods(BindingFlags.Public | BindingFlags.Static).Where(m => m.Name == name)).ToList();
        ...
        {
            List<MethodInfo> mis = type.GetMethods(bindingFlags).Where(m => m.Name == name).ToList();
        ...
        public static MethodInfo GetMethodWithParametersOrAttribute(this Type type, string name, BindingFlags bindingFlags, ParameterInfo[] parameters, Type attributeType = null)
        {
            ...
            if (methodInfo == null)
            {
                ...
                var extensionTypes = type.Assembly.GetTypes()
                    .Where((t) => t.IsSealed && !t.IsGenericType && !t.IsNested).ToArray();

                foreach (var extensionType in extensionTypes)
                {
                    methodInfos = extensionType.GetMethods(bindingFlags).Where(m => m.Name == name).ToArray();
                    if (!methodInfos.IsNullOrEmpty())
                    {
                        methodInfo = methodInfos.Select(
                            m => new
        ...
        {
            MethodInfo mi = type.GetMethods().FirstOrDefault((m) => m.MetadataToken == methodToken);
            return mi;
        }
        ...
        public static MethodInfo GetSimilarMethod(this Type type, MethodInfo methodInfo, BindingFlags bindingFlags, Type attributeType = null)
        {
            ...
            var methods = type.GetMethods(bindingFlags).Where(m => m.Name == methodInfo.Name).ToList();
            ...
            if (returnValue == null)
            {
                var extensionMethods = type.Assembly.GetTypes()
                    .Where((t) => t.IsSealed && !t.IsGenericType && !t.IsNested)
                    .SelectMany((t) => type.GetMethods(bindingFlags).Where(m => m.Name == methodInfo.Name)).ToList();
